...
static int flexcan_probe(struct platform_device *pdev)
{
	...
	if (PTR_ERR(reg_xceiver) == -EPROBE_DEFER)
		...
		reg_xceiver = NULL;
//add by lsb 20190327
	CAN_GPIO = of_get_named_gpio(pdev->dev.of_node, "gpios", 0);
        if (CAN_GPIO == -EPROBE_DEFER)
                return CAN_GPIO;
        if (CAN_GPIO < 0) {
                dev_err(&pdev->dev, "error can ctl gpio: %d\n", CAN_GPIO);
                return CAN_GPIO;
        }
	ret = devm_gpio_request_one(&pdev->dev, CAN_GPIO, 0, "can-gpio");
        if(ret) {
                dev_err(&pdev->dev, "error can ctl gpio: %d\n", ret);
                return ret;
        }

        gpio_direction_output(CAN_GPIO, 0);
//end add
	if (pdev->dev.of_node)
