...
void ipu_dump(struct ipu_soc *ipu)
{
	...
	printk("IPU_CONF = \t0x%08X\n",
        ...
        printk("IDMAC_CONF = \t0x%08X\n",
                ipu_idmac_read(ipu, IDMAC_CONF));
        printk("IDMAC_CHA_EN1 = \t0x%08X\n",
                ipu_idmac_read(ipu, IDMAC_CHA_EN(0)));
        printk("IDMAC_CHA_EN2 = \t0x%08X\n",
                ipu_idmac_read(ipu, IDMAC_CHA_EN(32)));
        printk("IDMAC_CHA_PRI1 = \t0x%08X\n",
                ipu_idmac_read(ipu, IDMAC_CHA_PRI(0)));
        printk("IDMAC_CHA_PRI2 = \t0x%08X\n",
                ipu_idmac_read(ipu, IDMAC_CHA_PRI(32)));
        printk("IDMAC_BAND_EN1 = \t0x%08X\n",
                ipu_idmac_read(ipu, IDMAC_BAND_EN(0)));
        printk("IDMAC_BAND_EN2 = \t0x%08X\n",
                ipu_idmac_read(ipu, IDMAC_BAND_EN(32)));
        printk("IPU_CHA_DB_MODE_SEL0 = \t0x%08X\n",
                ipu_cm_read(ipu, IPU_CHA_DB_MODE_SEL(0)));
        printk("IPU_CHA_DB_MODE_SEL1 = \t0x%08X\n",
                ipu_cm_read(ipu, IPU_CHA_DB_MODE_SEL(32)));
        printk("IPU_FS_PROC_FLOW1 = \t0x%08X\n",
                ipu_cm_read(ipu, IPU_FS_PROC_FLOW1));
        printk("IPU_FS_PROC_FLOW2 = \t0x%08X\n",
                ipu_cm_read(ipu, IPU_FS_PROC_FLOW2));
        printk("IPU_FS_PROC_FLOW3 = \t0x%08X\n",
                ipu_cm_read(ipu, IPU_FS_PROC_FLOW3));
        printk("IPU_FS_DISP_FLOW1 = \t0x%08X\n",
                ipu_cm_read(ipu, IPU_FS_DISP_FLOW1));
        for (i = 0; i < 15; i++)
                printk("IPU_INT_CTRL(%d) = \t%08X\n", i,
                        ipu_cm_read(ipu, IPU_INT_CTRL(i)));
#endif
}
